$(document).ready(function () {
    ...
    slick_mobile($('.table__block--scroll'), settings_table, 769);
    ...
    // Скрытие шапки при прокрутке вниз и отображение при прокрутке наверх на мобильном
    var header = $('.header'),
        scrollPrev = 0;
    $(window).scroll(function () {
        ...
        if (scrolled > 100 && scrolled > scrollPrev) {
            if (!($('.header__call-btn').hasClass('active')) &&
                !$('.cart').hasClass('transform-in') &&
                !$('.header__input-label').hasClass('transform-in') &&
                !$('.header__wrapper-nav').hasClass('active')
            ) {
                header.addClass('out');
            }
        } else {
    ...
    $('.nav-menu__item-opening').on('click', function () {
        $('.nav-menu__item-opening').toggleClass('active');
        $('.nav-menu__sublist').toggleClass('active');
    });
    ...
    // способ отображения товаров в каталоге
    function products_view_type() {
        if ($("input[name='view']:checked").val() === 'list') {
            $('.products').addClass('list-view');
        }

        if ($("input[name='view']:checked").val() === 'blocks') {
    ...
    // рейтинг звездами в отзыве
    const ratingItemList = document.querySelectorAll('.add-review__rate-item');
    ...
    ratingItemArray.forEach(item =>
        item.addEventListener('click', () => {
            ...
            // и значение потом передать на бекенд
        })
    );
